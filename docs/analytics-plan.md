...
口径：Cloudflare 当前仍能返回的首条记录为 2026-04-07，历史汇总截止于精细采集启用前的 2026-07-31；排除 Cloudflare 标记的 bot，并强制 `requestHost = shenzhe.org`。站点虽然创建于 2026-01-08，但 1 月 8 日至 1 月 29 日已超出接口保留范围，无法恢复。GraphQL 使用 Adaptive Groups 数据集，长窗口数据会抽样/估算，因此下列数值用于趋势判断，不视为逐条精确日志。

- 导入看板的主域名历史汇总：256 pageviews，152 visits；Cloudflare UI 的 30 天聚合口径显示约 130 pageviews、70 visits。
...
- 加密 IP 等精细明细保留 30 天；每日定时任务会先写入永久日汇总，再删除明细，因此总量和趋势持续保留。登录 15 分钟内最多尝试 10 次；采集端过滤常见机器人并限制每个 IP 每分钟 60 次。
...
- `0003_historical_rollups.sql` 已写入 2026-04-07 至 2026-07-31 的 16 个非零日期，共 256 次页面浏览、152 次访问。
- `0004_article_titles.sql` 已按四个不超过 30 天的窗口恢复 7 篇历史文章：研究分享约 20 次；心情随笔、庸俗的使用理论是一种怯懦、狐仙民间史、人工智能与计算政治学、AI 知识库、读博日记各约 10 次。
- 历史汇总无法提供 IP、城市或站长本人过滤；文章标题与总阅读量可显示，但不能还原为逐次访问明细。
- 看板默认选择“全部历史”，把历史汇总、超过 30 天的长期日汇总和最近 30 天精细记录合并为一条趋势线。
- 新访问实时进入 D1，页面每 60 秒自动刷新；每日定时汇总会长期保存文章路径与标题，然后清理含 IP 的明细，正常情况下不需要人工维护。
