...
迁移由 Wrangler 按顺序应用；`0003_historical_rollups.sql` 写入历史汇总并建立永久日汇总表。博客埋点位于 `components/StatsTracker.js`。
...
上线后无需手工导入：博客访问实时进入 D1，Worker 每天先汇总超过 30 天的记录，再删除含 IP 的明细。`stats.shenzhe.org` 默认展示全部可用历史，也可切换到 24 小时、7 天或 30 天。
